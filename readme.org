...
- =mode="new_chat_image"= waits for a visible image download button; on timeout it sends one creative-license retry prompt *only if a prompt can be sent immediately*, waits once more, then downloads and opens a new chat on success.
