...
- =mode="new_chat_image"= waits for a visible image download button; on each timeout it sends the next retry prompt using the normal send wait logic (by default: the creative-license guidance, then =Generate!=), waits again, then downloads and opens a new chat on success.
...
- Filename normalization remains best-effort under =@grant none=, but the rename interceptor now stays armed across hidden/background-tab waits and only times out after =60000ms= of /visible/ page time.
