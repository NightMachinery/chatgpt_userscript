...
- =mode="new_chat_image"= waits for a visible image download button; on timeout it sends one creative-license retry prompt using the normal send wait logic, waits once more, then downloads and opens a new chat on success.
